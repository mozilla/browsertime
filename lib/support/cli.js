...
module.exports.parseCommandLine = function parseCommandLine() {
  let validated = yargs
    ...
    .option('chrome.android.activity', {
      describe:
        'Name of the Activity hosting the WebView.',
      group: 'chrome'
    })
    ...
    .option('firefox.android.package', {
      describe:
        'Run Firefox or a GeckoView-consuming App on your Android device. Set to org.mozilla.geckoview_example for default Firefox version. You need to run adb start-server before you start.',
      group: 'firefox'
    })
    .option('firefox.android.activity', {
      describe:
        'Name of the Activity hosting the GeckoView.',
      group: 'firefox'
    })
    ...
    .option('firefox.mozillaProPreferences', {
      describe:
        'Use Firefox best practice preferences from the Mozilla performance team. These will probably be default in the next major release.',
      type: 'boolean',
      group: 'firefox'
    })
    .option('selenium.url', {
    ...
    .option('processStartTime', {
      type: 'boolean',
      default: false,
      describe: 'Capture browser process start time (in milliseconds).'
    })
    .option('safari.ios', {
      default: false,
      describe:
  ...
  if (argv.android) {
    if (argv.browser != 'firefox') {
      ...
      // Default to GeckoView example.
      set(
        argv,
        'firefox.android.package',
        get(argv, 'firefox.android.package', 'org.mozilla.geckoview_example')
      );
      set(
        argv,
        'firefox.android.activity',
        get(argv, 'firefox.android.activity', 'org.mozilla.geckoview_example.GeckoViewActivity')
      );
    }
  }
