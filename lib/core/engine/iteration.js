...
const Android = require('./command/android.js');
...
class Iteration {
  ...
  async run(navigationScript, index) {
    ...
    const processVideo = recordVideo && options.visualMetrics;
    const videos = [];
    const result = [];
        ...
        for (let myVideo of videos) {
          try {
            ...
            for (let progress of [
              ...
              videoMetrics.visualMetrics[progress] = util.jsonifyVisualProgress(
